...
var canvas_init = function() {
	...
	if (wwidth > 1210) {
		...
		$('.canvas-container').each(function() {
			$(this).removeClass('col-lg-9').addClass('col-lg-8');
		});
	}
	var width = $('#canvas-wrapper').parent().width();
	...
	$('#canvas-wrapper').width(width);
	$('#canvas-wrapper').height(height);
...
function JLsystem(canvas_name) {
	...
	$(window).resize(function() {
		canvas_init();
		canvas = new JLCanvas(canvas_name, system);
		system.draw();
	});
	...
	function action_click(time) {
		...
		this.getCapture = function(index) {
			if (captures[index]) {
				...
				if (captures[index].gates[0]) {
					return captures[index].gates[0];
				}
	...
	var drawCapture = function(type) {
		...
		$('#' + canvas_name).addClass('mouse-pointing');

		var capture = system.capture(system.data);
		if (type.fan && capture.fans[0]) {

			var fan = New(JLFan, capture.fans[0]);
			fan.patternCircle = new JLPattern(true, "#5cb85c", system.toScreen(3));
			fan.patternLeaf = new JLPattern(true, "#5cb85c", system.toScreen(2), true, "#006400");
			canvas.drawFan(fan);
			fan = null;
			return;
		}
		if (type.gate && capture.gates[0]) {
			...
			var gate = New(JLGate, capture.gates[0]);
			gate.pattern = new JLPattern(true, "#5cb85c", system.toScreen(3));
		...
		if (type.node && capture.nodes[0]) {
			...
			node.pattern = new JLPattern(true, "#5cb85c", system.toScreen(3));
		...
		if (type.edge && capture.edges[0]) {
			...
			//edge.pattern = new JLPattern(null, "#5cb85c", sys.toScreen(30), true, "#5cb85c");
			canvas.drawEdge(edge, {
				pattern : new JLPattern(null, "#5cb85c", sys.toScreen(30), true, "#5cb85c")
			});
			canvas.drawEdge(edge, {
				pattern : new JLPattern(null, "#ffffff", system.toScreen(24), null, null)
			});

			canvas.drawArrow(edge, {
				pattern : new JLPattern(null, "#5cb85c", 1, true, "#5cb85c")
			})
			return;
		}
		$('#' + canvas_name).removeClass('mouse-pointing');
	...
	function controller() {
			...
			if (_this.type == JLEdge) {
				...
				if (click.getPoint(1)) {
					...
					if (node1 == node2 && node1) {
						alertMessage(10, '绘制两点重复', '<p><a id="redrawEdge" class="btn btn-primary">重新绘制</a></p>');
					} else if (click.getPoint(0).equal(click.getPoint(1))) {
						alertMessage(10, '捕捉两点重复', '<p><a id="redrawEdge" class="btn btn-primary">重新绘制</a></p>');
					} else {
						if (!node1) {
							node1 = New(JLNode, {
		...
		this.drawDelete = function() {
			var elements = system.getRelations(_this.draw_delete);
			var paint = new JLCanvas("canvas_drawing", system);
			paint.clear();
			paint.begin();

			elements.edge.eachDo(paint.drawEdge, {
				pattern : new JLPattern(null, '#d43f3a', system.toScreen(30), null, null)
			});
			elements.edge.eachDo(paint.drawEdge, {
				pattern : new JLPattern(null, "#ffffff", system.toScreen(24), null, null)
			});
			elements.node.eachDo(paint.drawNode, {
				pattern : new JLPattern(true, '#d43f3a', system.toScreen(2), null, null)
			});
			elements.fan.eachDo(paint.drawFan, {
				pattern : new JLPattern(true, "#d43f3a", system.toScreen(3), true, "#F8F8FF"),
				patternLeaf : new JLPattern(true, "#d43f3a", system.toScreen(2), true, "#d43f3a")
			});
			elements.gate.eachDo(paint.drawGate, {
				pattern : new JLPattern(true, "#d43f3a", system.toScreen(3), true, "#F8F8FF")
			});
		}
		...
		//前次操作的类型
		this.drawType = JLEdge;

		this.drawOn = function(obj) {

			$('#' + canvas_name).addClass('mouse-drawing');
			_this.draw = true;
			...
			if (obj.isA(JLEdge)) {
				...
				click.reset(2);
				_this.type = JLEdge;
				_this.drawType = JLEdge;
			}
			if (obj.isA(JLFan)) {
				$('#badge-draw').html('风机');
				click.reset();
				_this.type = JLFan;
				_this.drawType = JLFan;
			}
			if (obj.isA(JLGate)) {
				$('#badge-draw').html('构筑物');
				click.reset();
				_this.type = JLGate;
				_this.drawType = JLGate;
			}
		...
		this.drawOff = function() {
			...
			$('#' + canvas_name).removeClass('mouse-drawing');
			$('#button-draw').removeClass('active');
			$('#badge-draw').removeClass('active');
		}

		this.draw_edge = false;
		this.draw_fan = false;
		this.capture_element = false;
		this.draw_delete = null;
	...
	system.connectionCheck = function() {
		var edges = [];
		var i = 0;
		for (var edge in system.data.edges.getArray()) {
			var ID = system.data.edges[edge].ID;
			edges[ID] = system.data.edges[edge];
			i++;
		}
		var check = function(edge, edges) {
			var start = edge.sNode;
			var end = edge.eNode;
	...
	system.init = function() {
		...
		var move = function() {
			$('#' + canvas_name).addClass('mouse-moving');
		...
		var moveFinish = function() {
			...
			$('#' + canvas_name).removeClass('mouse-moving');

			/*if(system.controller.draw){
			 $('#' + canvas_name).addClass('mouse-drawing');
			 }*/
		}
	...
	system.capture = function(objects, point, distance) {
		...
		var gate_capture = function(g, p, d) {
			var dis = system.geometry.pointDistance(system.toScreen(g.coord), p);
			if (dis <= d) {
				return d;
			} else {
				return false;
		...
		for (var element in objects) {
			...
			if (element == "gates") {
				var dis = distance ? distance : system.parameter.track.defaultGate;
				var p = point ? point : mouse_position;
				var gates = obj.getArray();
				var tracks = new Array();
				for (var i in gates) {
					if (gate_capture(gates[i], p, dis)) {
						tracks.push(gates[i]);
					}
				}
				captured.gates = tracks;
			}
		}
		return captured;
	}
	...
	system.boundCheck = function() {
		...
		$('#canvas1').css('border', '2px solid #333');

		var co = system.toSystem(new JLCoord());
		if (co.x < 0) {
			system.position.x = 0;
			$('#canvas1').css('border-left', '2px solid #d2322d');
		}

		if (co.y < 0) {
			system.position.y = 0;
			$('#canvas1').css('border-top', '2px solid #d2322d');
		}
		...
		if (co.x > zoom.maxWidth) {
			...
			$('#canvas1').css('border-right', '2px solid #d2322d');
		}

		if (co.y > zoom.maxHeight) {
			system.position.y = zoom.maxHeight - canvas.height * system.scale;
			$('#canvas1').css('border-bottom', '2px solid #d2322d');
		}
	...
	system.zoomIn = function(co) {
		...
		if (zoom.index > zoom.minIndex) {
			...
			$('#system-scale').animate({
				opacity : 0.2
			}, 800, function() {
				$('#system-scale').css('opacity', 1)
			});
		}
		$('#system-scale').val("1 : " + system.scale);
		system.boundCheck();
		system.draw();
		click.change();
	}
	...
	system.getRelations = function(arg) {
		...
		if (arg.isA(JLEdge)) {
			...
			obj.gate = system.data.gates.findByEdge(arg.ID);
		...
		if (arg.isA(JLNode)) {
			...
			for (i in obj.edge) {
				...
				obj.gate = obj.gate.concat(system.getRelations(obj.edge[i]).gate);
			}
		}
		var object = {};
		var type = {
		...
		for (var i in obj) {
			object[i] = new JLObjectArray(obj[i], type[i]);
		}
	...
	system.del = function(data) {
		var elements = system.getRelations(data);
		if (elements.node.size) {
			elements.node.eachRemove();
		} else if (elements.edge.size) {
			elements.edge.eachRemove();
		} else {
			elements.fan.eachRemove();
			elements.gate.eachRemove();
		}
		system.init();
		system.draw();
	}
	...
	system.draw = function() {
		...
		if (system.controller.draw_delete) {
			system.controller.drawDelete();
		}
...
function JLCanvas(canvas_id, system) {
	...
	var paint = this;
	this.screen = canvas;
	this.offset = new JLCoord(parseInt($('#' + canvas_id).offset().left), parseInt($('#' + canvas_id).offset().top));
	...
	this.drawNode = function(node, args) {
	...
	this.drawEdge = function(edge, args) {
		//"#00bfff"#2ba2e7
		var pattern = edge.pattern ? edge.pattern : new JLPattern(null, "#2fa4e7", system.toScreen(30), true, "rgba(128,128,128,0.5)");
		var opt = null;
		if (args) {
			if (args.pattern) {
	...
	this.drawFan = function(fan, args) {
		var diameter = 22;
		...
		if (args) {
			pattern = args.pattern || pattern;
			patternLeaf = args.patternLeaf || patternLeaf;
			opt = args.opt || opt;
		}
	...
	this.drawGate = function(gate, args) {
		var diameter = 22;
		var pattern = gate.pattern ? gate.pattern : new JLPattern(true, "#4169E1", system.toScreen(3), true, "#F8F8FF");
		var opt;
		if (args) {
			pattern = args.pattern || pattern;
			opt = args.opt || opt;
	...
	this.drawArrow = function(edge, args) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (sync) {
				...
				alertMessage(result.status, result.message, result.button);
			}
