...
function register($username, $email, $password, $firstname, $lastname) {
	...
	try {
		...
		$sql = 'INSERT INTO UserProject(user_ID,project_ID,authority,create_time,create_uid,update_uid) VALUES (?,?,1,CURRENT_TIMESTAMP,?,?)';
		$sth = $dbh -> prepare($sql);
		$sth -> execute(array($uid, $pid, $uid, $uid));
		$dbc -> disconnect();
		return $pid;
	} catch(PDOException $e) {
		error($e -> getMessage());
...
function login($username, $password) {
	try {
		...
		if (!$result -> ID) {
			...
			$sql = 'INSERT INTO H_User(user_ID,access_time,agent,ip) VALUES (?,CURRENT_TIMESTAMP,?,?)';
			$sth = $dbh -> prepare($sql);
			$sth -> execute(array($result -> ID, $browser, $ip));
			$user -> login = (int)$dbh -> lastInsertId();
			$dbc -> disconnect();
			$dbc = NULL;
			return $user;
		}
